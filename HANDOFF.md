...
- **Separate endpoint.** `/v1/vision/alpr` (CPAI-ALPR-compatible: predictions with
  ...
  **As of v0.8.0, Blue Iris 5.9.9 does NOT use this endpoint** — it uses `POST /`
  via the Plate Recognizer SDK protocol. See v0.8.0 section.
...
**VERIFIED in v0.8.0.** `ALPREngine.recognize()` correctly reads fast-alpr's
result objects. Note: `ocr.confidence` is a list of per-character floats — see
v0.8.0 section for the fix.
...
## v0.8.0 — Blue Iris 5.9.9 ALPR, plate suppression, plate history modal
